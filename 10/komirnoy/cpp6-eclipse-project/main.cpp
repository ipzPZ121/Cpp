...
int stringCopy(const char *src, char *dest);
...
//	int countL = countLines(getKeyFileName(argv[1]));
...
bool equalWords(const char *word1, const char *word2) {
	if(word1 == NULL) {
		cout << endl;
		cout << "Function equalWords(const char *word1, const char *word2)"
				<< endl;
		cout << "I can\'t read argument const char *word1 because it\'s empty"
				<< endl;
		cout << endl;
	...
	if(word2 == NULL) {
		...
		cout << "Function equalWords(const char *word1, const char *word2)"
				<< endl;
		cout << "I can\'t read argument const char *word2 because it\'s empty"
				<< endl;
		cout << endl;
...
char ** getEncodesFromKey(const char *fileName) {
	...
	if(fileName == NULL) {
		...
		unsigned int SIZE = countLines(fileKey);
...
//				int cS = countSymbols(buf);
//				for(int j = 0; buf[j] != '-'; j++) {
//
//				}
//				encodes[i] = new char[cS + 1];
//				for(int j = 0; j < cS; j++) {
//					b[j] = buf[j];
//				}
//				b[a] = '\0';

				char **token = stringToken(buf, '-');

				stringCopy(token[0], encodes[i]);
			}
